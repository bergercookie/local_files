...
\begin{figure}
        ...
        \begin{subfigure}[b]{0.48\textwidth}
                \includegraphics[width=\textwidth]{<+output_gauss+>}
                \caption{<+Program output when the Gauss elimination method was
                used+>}
                \label{fig:<+output_gauss+>}
        \end{subfigure}%
        ~
        \begin{subfigure}[b]{0.48\textwidth}
                \includegraphics[width=\textwidth]{<+output_lu+>}
                \caption{<+Program output when the LU method was used+>}
                \label{fig:<+output_lu+>}
        \end{subfigure}
        \caption{<+Program Output when run with different system solving
        methods+>}
        \label{figs:<+program_outputs+>}
\end{figure}
...
% include _ in the code
\usepackage{underscore}

% ABSTRACT
\begin{abstract} % beginning
\renewcommand{\abstractname}{Executive Summary} % change the abstract name
\end{abstract}


% LISTINGSINPUT
\lstinputlisting{../matlab_codes/running_main.m}
...
% SUBFIGURES IN TABLE STYLE \begin{figure}
\usepackage{tabularx}
\begin{figure}
...
% range of equations
http://tex.stackexchange.com/questions/18986/is-there-a-standard-way-to-refer-to-a-range-of-equations
...
% put the figure in the exact place from top - useful for 2column docs
\begin{figure*}[t]
    \centering
    \includegraphics[trim=+3cm 0 0 -6cm,width=0.5\textwidth]{EMF_ex}
    \caption{Παραδείγματα χρήσης ΗΔΜ}
    \label{fig:lab2:EMF_ex}
\end{figure*}


% make a table fit a page - move it to the left
...
% remove any occurance of \textlatin command
:\\textlatin{\<\(\w\+\>\_s*\)*}
