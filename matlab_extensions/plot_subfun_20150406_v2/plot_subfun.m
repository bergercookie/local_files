...
%
%OTHER OPTIONS
%-nofilename    hides the file> part of each function name
...
data = sub_add_filebox(data);       %adds the file itself as a node
...
data.fun.used(1) = true;                %add function itself to used
    ...
    function sub_check_options(varargin)
        ...
        targ = {...
            ...
            '-extlist','-extshow','-filename','-nomain'};
    ...
    function data = sub_parse_file(data,foo)
        ...
        if ~exist(data.foo,'file');
            warning('plot_subfun:noFile',['File "' foo '" does not exist']);
        end
...
function data = sub_add_filebox(data)
...
%TODO       builtin option ?
%TODO       give self a file box
%TODO       get -unused and -unusedonly to work with -extsub
%TODO       plot_graph try to fill the screen (scale text ?)
%TODO       if main filename does not match function name, warn
...
%TODO       option -recurse to recurse through all dependencies
...
%TODO       -filedep function removes the main function in each file
...
%TODO       when hiding children of a "hide" function, never hide their
